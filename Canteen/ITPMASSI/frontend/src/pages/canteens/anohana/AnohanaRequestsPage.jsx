...
const AnohanaRequestsPage = () => {
  ...
  const handleSendRequest = async ({ helperId, quantity, serviceCharge, message, itNumber, phoneNumber }) => {
    ...
    try {
      ...
      // Offline fallback: save request locally and render immediately
      const fallbackRequest = {
        id: `local-${Date.now()}`,
        _id: `local-${Date.now()}`,
        requesterId: currentUser._id,
        foodName: selectedFood.name,
        itemPrice: Number(selectedFood.price || 0),
      ...
      setMyRequests((prev) => [fallbackRequest, ...prev]);
      setSelectedFood(null);
      showAlert("success", "Request saved locally (offline mode).");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-8 shadow-soft">
        <h2 className="text-3xl font-extrabold text-slate-900">ANOHANA - Requests & Tracking</h2>
        <p className="mt-2 text-slate-600">Main Building • Request food assistance and track order status</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {selectedHelper && (
          <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-6 shadow-soft">
            <p className="mb-3 text-sm font-bold text-emerald-900 flex items-center gap-2">
              <FiCheckCircle className="text-emerald-600" /> Selected Helper
            </p>
            <div className="space-y-2 rounded-xl bg-white p-4 border border-emerald-200">
            ...
            <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-emerald-700">LKR {Number(expandedHelperDetails.serviceCharge || 0).toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-700 transition"
                >
                  Select as Helper
                </button>
                <button
      ...
      <div className="rounded-2xl border border-slate-200 bg-amber-50 p-5 text-amber-800 shadow-soft">
...
export default AnohanaRequestsPage;
