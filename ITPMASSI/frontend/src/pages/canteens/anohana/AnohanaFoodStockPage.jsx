...
const ANOHANA_DEFAULT_FOODS = [
  {
    _id: "anohana-local-1",
    name: "Fried Rice",
    price: 550,
    inStock: true,
    image: "/images/canteen/fried-rice.jpg",
  },
  {
    _id: "anohana-local-2",
    name: "Rice & Curry",
    price: 500,
    inStock: true,
    image: "/images/canteen/rice-curry.jpg",
  },
  {
    _id: "anohana-local-3",
    name: "Pizza",
    price: 950,
    inStock: true,
    image: "/images/canteen/pizza.jpg",
  },
  {
    _id: "anohana-local-4",
    name: "Roll",
  ...
  },
  {
    ...
    price: 100,
    inStock: true,
    image: "/images/canteen/samosa.jpg",
  },
  {
    _id: "anohana-local-6",
    name: "Chicken Roll",
    price: 280,
    inStock: false,
    image: "/images/canteen/chicken-roll.jpg",
  },
];

const AnohanaFoodStockPage = () => {
  const LOCAL_REQUESTS_KEY = "anohana-local-requests";
  const ACTIVE_USER_KEY = "anohana-active-user-id";
  ...
  const loadFoods = async (silent = false) => {
    ...
    try {
      ...
      // Always include 5 local Anohana image cards, then append API data.
      setFoods([...ANOHANA_DEFAULT_FOODS, ...serverFoods]);
    } catch {
      setFoods(ANOHANA_DEFAULT_FOODS);
    } finally {
      if (!silent) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-emerald-50/30 p-8 shadow-soft">
        <div className="absolute top-0 right-0 opacity-10">
          <FiBox className="text-9xl text-emerald-600" />
        </div>
        <div className="relative z-10">
          <h2 className="text-4xl font-black text-slate-900">ANOHANA - Food & Stock</h2>
          <p className="mt-2 text-slate-600">Main Building • Premium food selection with real-time availability</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft hover:shadow-md transition">
          <div className="flex items-center justify-between">
            ...
            <div className="rounded-xl bg-gradient-to-br from-emerald-100 to-emerald-50 p-4 text-3xl">
              🍽️
            </div>
          </div>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-emerald-100/30 p-6 shadow-soft hover:shadow-md transition">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest font-semibold text-emerald-700">In Stock</p>
              <p className="mt-2 text-4xl font-black text-emerald-700">{inStockCount}</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 p-4 text-2xl text-white">
              ✓
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-amber-100/30 p-6 shadow-soft hover:shadow-md transition">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest font-semibold text-amber-700">Avg Price</p>
              <p className="mt-2 text-3xl font-black text-amber-700">LKR {avgPrice}</p>
            </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full rounded-xl border border-slate-200 py-3 px-4 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100 transition"
            />
          </div>

          <div className="flex gap-2">
            <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-600 cursor-pointer hover:bg-emerald-50 hover:border-emerald-300 transition font-semibold">
              <input
                type="checkbox"
            ...
            <button
              type="button"
              onClick={() => loadFoods()}
              className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 px-4 py-3 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 hover:border-emerald-400 transition"
            >
              <FiRefreshCw /> Refresh
            </button>
          </div>
        ...
        {loadingFoods ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredFoods.map((food) => (
              <div
                key={food._id}
                className="group rounded-2xl border border-slate-200 bg-white overflow-hidden hover:shadow-xl hover:border-emerald-300 transition-all duration-300"
              >
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="space-y-2">
                    <h4 className="font-bold text-slate-900 group-hover:text-emerald-700 transition text-base line-clamp-2 leading-tight">
                      {food.name}
                    </h4>
                    <div className="flex items-end gap-2">
                      <p className="text-3xl font-black text-emerald-600">LKR {Number(food.price).toFixed(2)}</p>
                  ...
                  {food.inStock && (
                    <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-xl border border-emerald-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-emerald-700 font-semibold">Available Now</p>
                        <p className="text-sm font-bold text-emerald-900">{food.quantity} in stock</p>
                      </div>
                    </div>
                  )}
          ...
          <div className="w-full max-w-xl rounded-2xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-200 p-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">{selectedFood.category.label} • Anohana Canteen</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2">
              <div className="rounded-xl bg-emerald-50 p-4">
                ...
                <p className="mt-1 text-2xl font-extrabold text-emerald-800">LKR {Number(selectedFood.price).toFixed(2)}</p>
              </div>
              ...
              <div className="rounded-xl bg-brand-50 p-4">
                ...
                <p className="mt-1 text-lg font-bold text-brand-800">{selectedFood.category.emoji} {selectedFood.category.label}</p>
              </div>
            </div>
          </div>
        </div>
...
export default AnohanaFoodStockPage;
