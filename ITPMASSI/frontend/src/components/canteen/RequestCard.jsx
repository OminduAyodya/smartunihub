    ...
    <article className="rounded-xl border border-slate-100 bg-slate-50/80 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span
          className={`rounded-full px-2 py-1 text-xs font-semibold ${
            statusClass[request.status] || "bg-slate-100 text-slate-600"
          }`}
        >
          ...
          {statusSteps.map((step, idx) => {
            ...
            return (
              <div key={step} className="flex flex-col items-center flex-1">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold m b-1 transition ${
                  isCompleted ? (isActive ? "bg-blue-600" : "bg-emerald-600") : "bg-slate-300"
                }`}>
                  {isCompleted && !isActive ? "✓" : idx + 1}
                </div>
                <span className={`mt-0.5 text-center font-semibold capitalize text-xs ${
      ...
      {request.status === "accepted" ? (
        <div className="mt-2 space-y-2">
          ...
          {helperLocation && (
            <div className="rounded-lg border-2 border-blue-300 bg-gradient-to-r from-blue-50 to-blue-100 px-3 py-3 text-xs text-blue-800 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-1">
                  <p className="flex items-center gap-2"><span className="font-semibold">Distance:</span> {helperLocation.distance}</p>
                  <p className="flex items-center gap-2"><span className="font-semibold">ETA:</span> {helperLocation.eta}</p>
                </div>
              </div>
            </div>
          )}
